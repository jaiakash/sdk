## What's Changed
* feat: added git cliff git eg for changelog
* Kubeflow SDK Official Release 0.1.0 by @kramaranya
* fix(scripts): Use previous stable tag for changelog by @kramaranya
* Kubeflow SDK Official Release 0.1.0rc1 by @kramaranya
* chore: Ignore PRs titles with area/release labels in CI by @kramaranya
* feat(ci): Add automated release CI job by @kramaranya
* fix(docs): Update links before SDK release by @kramaranya
* chore: Add proper ruff configuration by @szaher
* feat: Implement TrainerClient Backends & Local Process by @szaher
* chore: Update CONTRIBUTING.md to use uv by @szaher
* feat: KEP-2 Local Execution Mode Proposal by @szaher
* chore: Add welcome new contributors CI by @kramaranya
* feat(trainer): Add support for param unpacking in the training function call by @briangallagher
* feat: Support multiple pip index URLs in CustomTrainer by @wassimbensalem
* feat(trainer): Refactor get_job_logs() API with Iterator by @andreyvelich
* chore(trainer): Use explicit exception chaining by @andreyvelich
* chore: Nominate @kramaranya and @szaher as Kubeflow SDK reviewers by @andreyvelich
* chore: Enable parallel builds for coveralls by @kramaranya
* fix: trainer client backend public by @jaiakash
* chore: Remove tool.hatch.build.targets from pyproject by @kramaranya
* chore: Move dev extras to dependency-groups by @kramaranya
* chore: Update README.md by @kramaranya
* feat: Implement Kubernetes Backend by @szaher
* fix(trainer): Keep the original runtime command in get_runtime_packages() API by @andreyvelich
* feat(docs): add ROADMAP of Kubeflow SDK by @kramaranya
* chore: move pyproject.toml to root by @kramaranya
* feat(trainer): Add `get_runtime_packages()` API by @andreyvelich
* chore(ci): Align Kubernetes versions from Trainer for e2e tests by @astefanutti
* chore(ci): Add dev tests with master dependencies by @kramaranya
* feat(trainer): Support Framework Labels in Runtimes by @andreyvelich
* feat(trainer): Add environment variables argument to CustomTrainer by @astefanutti
* feat(trainer): Add `wait_for_job_status()` API by @andreyvelich
* chore(docs): Add Coveralls Badge to the README by @andreyvelich
* chore(trainer): Remove accelerator label from the runtimes by @andreyvelich
* fix(trainer): fix __all__ import. by @Electronic-Waste
* feat(ci): Add GitHub action to verify PR titles by @andreyvelich
...
* fix: Expose BuiltinTrainer API to users by @Electronic-Waste
...
* KEP-2401: Support mutating dataset preprocessing config in SDK (kubeflow/trainer#2638) by @Electronic-Waste
* Revert "fix(sdk): Fix type annotation for `train` method's `trainer` parameter" (kubeflow/trainer#2651) by @Electronic-Waste
* fix(sdk): Fix bad arg passed to `get_args_using_torchtune_config` (kubeflow/trainer#2647) by @eoinfennessy
* fix(sdk): Fix type annotation for `train` method's `trainer` parameter (kubeflow/trainer#2646) by @eoinfennessy
...
* KEP-2401: Complement torch plugin to support torchtune config mutation (kubeflow/trainer#2587) by @Electronic-Waste
* feat(sdk): Get namespace from the provided context (kubeflow/trainer#2593) by @andreyvelich
* feat(runtimes): Support MLX Distributed Runtime with OpenMPI (kubeflow/trainer#2565) by @andreyvelich
* KEP-2401: Add `TorchTuneConfig` to `train()` API (kubeflow/trainer#2522) by @Electronic-Waste
* feat(runtimes): Support DeepSpeed Runtime with OpenMPI (kubeflow/trainer#2559) by @andreyvelich
* fix(sdk): Add missing import types. (kubeflow/trainer#2566) by @Electronic-Waste
* fix(sdk): Using correct entrypoint for mpirun (kubeflow/trainer#2552) by @andreyvelich
* feat(sdk): Support MPI-based TrainJobs  (kubeflow/trainer#2545) by @andreyvelich
* fix(sdk): add missing import type Initializer. (kubeflow/trainer#2541) by @Electronic-Waste
* feat(controller): Refactor the Initializer APIs of TrainJob (kubeflow/trainer#2523) by @andreyvelich
* Fix kubeflow/trainer#2407: Cap nproc_per_node based on CPU resources for PyTorch TrainJob by @Diasker
* KEP-2401: Refactor current `train()` API (kubeflow/trainer#2513) by @Electronic-Waste
* feat(sdk): Migrate to OpenAPI V3 (kubeflow/trainer#2490) by @andreyvelich
* feat(controller): Integrate DependsOn API (kubeflow/trainer#2484) by @andreyvelich
* Implement MPI numProcPerNode defaulter (kubeflow/trainer#2483) by @tenzen-y
* feat(sdk): Generate external Kubernetes and JobSet models (kubeflow/trainer#2466) by @andreyvelich
* chore(test): Add E2E tests for Kubeflow Trainer (kubeflow/trainer#2470) by @andreyvelich
* fix(sdk): resolve errors in deserialization (kubeflow/trainer#2457) by @Electronic-Waste
...
* KEP-2170: Add validation to Torch `numProcPerNode` field (kubeflow/trainer#2409) by @astefanutti
* KEP-2170: Implement MPI Plugin for Kubeflow Trainer (kubeflow/trainer#2394) by @andreyvelich
...
* V1: Fix versions in HuggingFace dataset initializer (kubeflow/trainer#2369) by @andreyvelich
...
* KEP-2170: [SDK] Initial implementation of the Kubeflow Training V2 Python SDK (kubeflow/trainer#2324) by @andreyvelich
...
* E2E: Replace outdated images with latest ones (kubeflow/trainer#2083) by @tenzen-y
* [SDK] Add docstring for Train API (kubeflow/trainer#2075) by @andreyvelich
* SDK: Upgrade the minimum required Kubernetes version to v1.27.2 (kubeflow/trainer#2066) by @tenzen-y
...
* feat(pytorch): Support elastic training (kubeflow/trainer#1453) by @gaocegege
...
* fix: Add missing retrying package that failed the import (kubeflow/trainer#1439) by @terrytangyuan
...
* Fix: the "follow" of TFJobClient.get_logs (kubeflow/trainer#1254) by @Windfarer
